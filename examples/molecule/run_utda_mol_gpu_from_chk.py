...
def make_gpu_uks_from_chk(chk, xc, use_density_fit=True, auxbasis=None):
    ...
    if mf.mo_coeff.ndim != 3 or mf.mo_occ.ndim != 2:
        raise ValueError("UTDA example expects a UKS checkpoint; use UKS.chk.")
    return mf
...
def main():
    ...
    ee, vv = utda_method.kernel(nstates=nstates, save=save_results, save_file=save_file)
    ...
    if run_analyse:
        utda_method.analyse(compute_s2=analyse_delta_s2)
