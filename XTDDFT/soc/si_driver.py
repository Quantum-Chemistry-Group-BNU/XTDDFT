...
from ..base import _molecular_ground_dipole
from ...utils.backend import asnumpy, get_array_module, require_cupy, resolve_backend
from ...utils.unit import au2D, eV2cm_1, ha2eV
...
def symm_matrix(h):
    ...
    return xp.triu(h) + xp.triu(h, 1).T.conjugate()
...
class SI_driver():
    ...
    Args:
        ...
        tdm_blocks: transition dipole blocks {'gs_so', 'so', 'sm', 'sp'}
                    computed by the local XTDA/XSF_TDA_down/SF_TDA_up
                    classes. Required when cal_osc=True.
        backend: 'auto'/'cpu'/'gpu'; selects the array backend for the
                 state-interaction einsums (auto follows utils.backend).
    '''
    def __init__(
            self,
            mf: Optional[Union[
        ...
        if self.cal_osc:
            if tdm_blocks is None:
                raise ValueError("tdm_blocks is required when cal_osc=True")
            self._tdm_gs_so = self.xp.asarray(tdm_blocks["gs_so"])
            self._tdm_so = self.xp.asarray(tdm_blocks["so"])
            self._tdm_sm = (None if tdm_blocks.get("sm") is None
                            else self.xp.asarray(tdm_blocks["sm"]))
            self._tdm_sp = self.xp.asarray(tdm_blocks["sp"])
        ...
        if int(ngs) == 0:
            ...
            self.states['|GS>'] = [(0.0, self.xp.array([1.0]))]
        for key in ('|So>', '|S+>', '|S->'):
    ...
    def kernel(self,printnum=100):
        ...
        '''
        logger.info(f"Perform SI calculation with Si={self.S}")
        time0 = time.time()
        self.heff = asnumpy(self.make_heff())
        self.hso = asnumpy(self.hso)
        if self.cal_osc:
            self.dm = asnumpy(self.dm)
        self.Omega = asnumpy(self.Omega)
        logger.info(f"Begin to diag Omega+hso --{self.heff.shape}--")
        logger.info(f"{'='*20} Heff diagonalization {'='*20}")
        time1 = time.time()
        # heff is tiny (<~300); keep the final eigh on CPU for exact
        # reproducibility while the heavy einsums run on the selected backend.
        import scipy
        ...
        logger.info(f"End SI calculation, cost time {self.codetime:.2f}s")
        self.summary(printnum)
        # self.summary_osc(20)
        return self.eso, self.vso
    
    def summary(self,printnum=100):
        logger.info(f"{'='*16} Basic setting {'='*16}")
        logger.info(f"Time cost {self.codetime:.2f}s")
        logger.info(f"|Ref⟩ with S = Sz = {self.S}")
        logger.info(f"|Ref⟩ with E (in Hatree) = {self.mf.e_tot}")
        logger.info(f"Interaction among {self.nSm} |S-⟩, {self.ngs} |GS⟩, {self.nSo} |So⟩, {self.nSp} |S+⟩.")
        logger.info(f"{'='*18} Summary of S I calculation {'='*18}")
        ...
        for i, e in enumerate(self.eso[:printnum]):
            ...
            print(f"{i:4d} {str_state}  {e_sf*ha2eV:>12.8f}  {self.eso[i]*ha2eV:>12.8f}     {((self.eso[i]-self.eso[0])*ha2eV):>10.8f}    {((self.eso[i]-self.eso[0])*ha2eV*eV2cm_1):>10.2f}")
    
    def summary_osc(self,printnum=100,unit='eV'):
        logger.info(f"{'='*16} Dipole moments and Oscillator strength {'='*16}")
    ...
    def print_hso_local(self,SML,SMR):
        ...
        print(f"{marker} = {hso:.8f} Ha. = {hso*ha2eV:.8f} eV = {hso*ha2eV*eV2cm_1:.8f} cm-1")
    ...
    def cal_hm(self, Vso):
        ...
        Transfer Vso to hm
        '''
        Vso = self.xp.asarray(Vso)
        hm = self.xp.zeros((self.mol.nao,self.mol.nao,3,), dtype=np.complex128)
    ...
    def cal_dims(self) -> None:
        ...
        # delta
        self.delta_c = self.xp.eye(self.nc)
        self.delta_o = self.xp.eye(self.no)
        self.delta_v = self.xp.eye(self.nv)
        ...
        logger.info(f"Slice of orbitals:")
        ...
        logger.info(f"Slice of |S-⟩ is")
        logger.info(f"|S-⟩ CV(1):   Slice {self.sl_S_1_dim0}, length: {self.cv}")
        logger.info(f"|S-⟩ CO(1):   Slice {self.sl_S_1_dim1}, length: {self.co}")
        logger.info(f"|S-⟩ OV(1):   Slice {self.sl_S_1_dim2}, length: {self.ov}")
        logger.info(f"|S-⟩ O1O2(1): Slice {self.sl_S_1_dim3}, length: {self.oo}")
        logger.info(f"|S-⟩ O1O1(1): Slice {self.sl_S_1_dim4}, length: {self.no}")
        ## |S>
        ...
        logger.info(f"Slice of |So⟩ is")
        logger.info(f"|So⟩ CV(0):   Slice {self.sl_S_dim0}, length: {self.cv}")
        logger.info(f"|So⟩ CO(0):   Slice {self.sl_S_dim1}, length: {self.co}")
        logger.info(f"|So⟩ OV(0):   Slice {self.sl_S_dim2}, length: {self.ov}")
        logger.info(f"|So⟩ CV(1):   Slice {self.sl_S_dim3}, length: {self.cv}")
        ## |S+>
        logger.info(f"Slice of |S+⟩ is")
        self.dim_Sp = self.cv
        logger.info(f"|S+⟩ CV(1):                           length: {self.cv}")
        # hso pos
        self.nGS = self.ngs
    ...
    def cal_hso_pos(self,SL,ML,Li,igsL,SR,MR,Ri,igsR,debug=0):
        ...
        if debug:
            logger.debug(f"(SL,ML,Li,igsL,SR,MR,Ri,igsR) = {(SL,ML,Li,igsL,SR,MR,Ri,igsR)}")
            logger.debug(f"state={L_index}, begin={L_dim}, {int(L_Mnum)}-M, foot-length={L_length}")
        return (L_pos, R_pos)
    ...
    def cal_pos(self,pos):
        ...
        if 0<=pos<self.dim0: # |S->
            ...
            raise ValueError(f"position {pos} out of range (dim_hso={self.dim_hso})")
        M = -S + M
        return (S,M,ith,igs)
    ...
    def make_heff(self):
        ...
        '''
        logger.info(f"{'='*20} Begin to make heff {'='*20}")
        logger.info(f"dim of Heff = {self.dim_hso}")
        time0 = time.time()
        if self.cal_osc:
            self.dm = self.xp.zeros((self.dim_hso, self.dim_hso,3,))
        hso = self.xp.zeros((self.dim_hso, self.dim_hso,),dtype=np.complex128)
            ...
            for Li, L_value in enumerate(self.states[L_index]): # [L_value]: Different eigenvecter for fixed SL
                ...
                for R_index in States_list: # [R_index]: |S->, |GS>, |So>, |S+> Different [SR]
                    ...
                    for Ri, R_value in enumerate(self.states[R_index]): # [R_value]: Different eigenvecter for fixed SR
                        ...
                        if self.cal_osc:
                            d = self.make_dm_local(SL,SR,Li,Ri,L,R) # <Phi|Ol|Phi>
                        for ML in self.xp.arange(-SL,SL+1,1): # Different [ML] for fixed SL
                            for MR in self.xp.arange(-SR,SR+1,1): # Different [MR] for fixed SR
                                igsL, igsR = 0, 0
                                if L[0] == '|GS>': igsL = 1
                                if R[0] == '|GS>': igsR = 1
                                ...
                                if Lpos <= Rpos: # For only upper triangle
                                    # Debug
                                    # logger.debug(f"(SL,ML,Li,igsL,SR,MR,Ri,igsR) = {(SL,ML,Li,igsL,SR,MR,Ri,igsR)}")
                                    # logger.debug(f"Lpos={Lpos}({L_index}, {float(ML)}, {Li}-th),\
                                    #                 Rpos={Rpos}({R_index}, {float(MR)}, {Ri}-th)")
                                    # -----------------------------------------------------
                                    # make sure that hso_so is calculated only once.
                                    if not exitcode == 1:
                                        logger.warning(f"exitcode={exitcode}, check hso!")
        ...
        self.hso = self.hso - self.xp.diag(self.xp.diag(self.hso))
        self.Omega = Omega
        self.heff = self.hso + self.Omega
        time1 = time.time()
        ndiff_hso = self.xp.linalg.norm(self.hso-self.hso.T.conjugate())
        ndiff_heff = self.xp.linalg.norm(self.heff-self.heff.T.conjugate())
        logger.info(f"Norm(Hso-Hso.dagger)={ndiff_hso:.2e}")
        logger.info(f"Norm(Heff-Heff.dagger)={ndiff_heff:.2e}")
        # if ndiff_heff > 1e-6: breakpoint()
        logger.info(f"End of contructing heff, cost time {time1-time0:.2f}s")
        return self.heff
    ...
    def interact_S_1S_1(self, L, R):
        ...
        hX = self.xp.zeros((dim,3,), dtype=np.complex128)
        ...
        factor = -(self.S-1)/sqrt(self.S*(2*self.S-1))
    ...
    def interact_S_1S(self, L, R):
        ...
        factor = sqrt((2*self.S-1)/(2*self.S))
        hX[self.sl_S_1_dim2] += factor*einsum('abm,ut,tb->uam', self.hm[self.slv,self.slv,:],self.delta_o, XR).reshape(self.ov,3)
        hX[self.sl_S_1_dim2] += factor*einsum('tum,ab,tb->uam', self.hm[self.slo,self.slo,:]/(2*self.S-1),self.delta_v, XR).reshape(self.ov,3)
    ...
    def interact_S_1S1(self, L, R):
        XhX = self.xp.zeros((3,), dtype=np.complex128)
        return XhX
    ...
    def interact_SS(self, L, R):
        # XL-- S [CV CO OV CV]
        XL = L[1][1]
        hX = self.xp.zeros((XL.shape[0],3,), dtype=np.complex128)
        # ====== line0 ==========
    ...
    def make_dm_local(self, SL, SR, Li, Ri, L, R):
        '''
        Transition dipole <Phi_L|O^l|Phi_R> from precomputed local blocks.
        SL, SR: spin quantum numbers of the two state blocks.
        Li, Ri: state indices inside the blocks.
        L, R: (block_name, (energy, X)) tuples, kept for the |GS> flags.
        '''
        if abs(SL - SR) > 1e-9:
            return self.xp.zeros(3)
        igsL = L[0] == '|GS>'
        igsR = R[0] == '|GS>'
        if igsL or igsR:
            if igsL and igsR:
                return self.xp.zeros(3)
            if igsL:
                return self.xp.asarray(self._tdm_gs_so[Ri])
            return self.xp.asarray(self._tdm_gs_so[Li]).conj()
        if abs(SL - (self.S - 1)) < 1e-9:
            return self.xp.asarray(self._tdm_sm[Li, Ri])
        if abs(SL - (self.S + 1)) < 1e-9:
            return self.xp.asarray(self._tdm_sp[Li, Ri])
        return self.xp.asarray(self._tdm_so[Li, Ri])
