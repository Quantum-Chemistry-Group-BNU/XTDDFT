...
# def get_soDKH1_somf(myhf,mol,c,iop='x2c',include_mf2e=True,mf2e_impl='auto',nproc=1,use_1c=True,debug=False):
# 
"""Spin-free X2C + SOMF spin-orbit Hamiltonian (sf-X2C + DKH1).

Original implementation by Zhendong Li; see JCP 137, 154114 (2012),
JCP 141, 054111 (2014) and Mol. Phys. 111, 3741 (2013).
"""
...
def _estimate_full_mf2e_memory_bytes(nb, dtype=np.float64):
    '''
...
def inv12(s):
    ...
    return reduce(np.dot,(v,np.diag(1/np.sqrt(e)),v.T))
...
def sfx2c1e(t, v, w, s, c):
    ...
    cl = a[:nao,nao:]
    cs = a[nao:,nao:]
    x = cs.dot(cl.T.dot(np.linalg.inv(cl.dot(cl.T)))) # ref.(40)
    stilde = s + x.T.dot(m[nao:,nao:].dot(x)) # ref.(39) and here m[nao:,nao:] = t * (.5/c**2) is t·a^2/2
    sih = inv12(s) # s^{-1/2}
    sh = np.linalg.inv(inv12(s)) # s^{1/2}
...
def get_kint(mol,debug=0):
    ...
    '''
    nb = mol.nao_nr()
    npair = nb*nb # number of excitations
    nq = npair*npair # two single excitation
    ...
    if debug:
        for n in range(0,kint.shape[0]):
            logger.info(f"axis-{n}, kint+kint.T={np.linalg.norm(kint[n]+kint[n].transpose(3,4,1,2))}")
    return kint
...
def get_hso1e(wso,x,rp):
    ...
    '''
    nb = x.shape[0]
    hso1e = np.zeros((3,nb,nb))
    for ic in range(3):
        hso1e[ic] = reduce(np.dot,(rp.T,x.T,wso[ic],x,rp))
    return hso1e
...
def get_fso2e(kint,x,rp,pLL,pLS,pSS):
    ...
    '''
    nb = x.shape[0]
    fso2e = np.zeros((3,nb,nb))
    for ic in range(3):
        gsoLL  = -np.einsum('lmkn,lk->mn',kint[ic],pSS)*2.

        gsoLS  = -np.einsum('mlkn,lk->mn',kint[ic],pLS)
        gsoLS -=  np.einsum('lmkn,lk->mn',kint[ic],pLS)

        gsoSS  = -np.einsum('mnkl,lk->mn',kint[ic],pLL)*2.
        gsoSS -=  np.einsum('mnlk,lk->mn',kint[ic],pLL)*2.
        gsoSS +=  np.einsum('mlnk,lk->mn',kint[ic],pLL)*2.

        fso2e[ic] = gsoLL + gsoLS.dot(x) + x.T.dot(-gsoLS.T) + x.T.dot(gsoSS.dot(x))
        fso2e[ic] = reduce(np.dot,(rp.T,fso2e[ic],rp))
    return fso2e

def _atom_ao_slices(mol):
    ao_loc = mol.ao_loc_nr() # 获取基函数的位置索引（ao_loc[i] = 第 i 个壳层的第一个基函数下标）
    atom_slices = []
    for ia in range(mol.natm):
        shell_ids = [ish for ish in range(mol.nbas) if mol.bas_atom(ish) == ia]
        if not shell_ids:
            continue
        atom_slices.append((ao_loc[shell_ids[0]], ao_loc[shell_ids[-1] + 1]))
    return atom_slices

def get_fso2e_1c(kint,mol,x,rp,pLL,pLS,pSS):
    '''
    Full-K contraction with the one-center approximation. Only AO quartets
    whose four functions belong to the same atom are retained.
    '''
    nb = x.shape[0]
    gsoLL = np.zeros((3,nb,nb))
    gsoLS = np.zeros((3,nb,nb))
    gsoSS = np.zeros((3,nb,nb))
    for a0, a1 in _atom_ao_slices(mol):
        for ic in range(3):
            ksub = kint[ic, a0:a1, a0:a1, a0:a1, a0:a1]
            gsoLL[ic, a0:a1, a0:a1] += -2. * np.einsum(
                'lmkn,lk->mn', ksub, pSS[a0:a1,a0:a1]
            )
            gsoLS[ic, a0:a1, a0:a1] += -np.einsum(
                'mlkn,lk->mn', ksub, pLS[a0:a1,a0:a1]
            )
            gsoLS[ic, a0:a1, a0:a1] += -np.einsum(
                'lmkn,lk->mn', ksub, pLS[a0:a1,a0:a1]
            )
            gsoSS[ic, a0:a1, a0:a1] += -2. * np.einsum(
                'mnkl,lk->mn', ksub, pLL[a0:a1,a0:a1]
            )
            gsoSS[ic, a0:a1, a0:a1] += -2. * np.einsum(
                'mnlk,lk->mn', ksub, pLL[a0:a1,a0:a1]
            )
            gsoSS[ic, a0:a1, a0:a1] += 2. * np.einsum(
                'mlnk,lk->mn', ksub, pLL[a0:a1,a0:a1]
            )
    return _finalize_fso2e(gsoLL,gsoLS,gsoSS,x,rp)

def _shell_groups_for_1c(mol, ish0, ish1):
    groups = []
    for ia in range(mol.natm):
        shells = [
            ish for ish in range(mol.nbas)
            if mol.bas_atom(ish) == ia
        ]
        local_shells = [ish for ish in shells if ish0 <= ish < ish1]
        if local_shells:
            groups.append((local_shells, shells))
    return groups

def _block_groups_for_1c(mol, blocks, ib0, ib1):
    groups = []
    for ia in range(mol.natm):
        block_ids = [
            ib for ib, (sh0, _) in enumerate(blocks)
            if mol.bas_atom(sh0) == ia
        ]
        local_blocks = [ib for ib in block_ids if ib0 <= ib < ib1]
        if local_blocks:
            groups.append((local_blocks, block_ids))
    return groups

def _accumulate_gso_shell_range(mol,pLL,pLS,pSS,ish0,ish1,use_1c=False,xp=None):
    xp = np if xp is None else xp
    nb = pLL.shape[0]
    gsoLL = xp.zeros((3,nb,nb))
    gsoLS = xp.zeros((3,nb,nb))
    gsoSS = xp.zeros((3,nb,nb))
    ao_loc = mol.ao_loc_nr()
    if use_1c:
        shell_groups = _shell_groups_for_1c(mol, ish0, ish1)
    else:
        shell_groups = [(range(ish0, ish1), range(mol.nbas))]
    for local_shells, shells in shell_groups:
        for ish in local_shells:
...
def _finalize_fso2e(gsoLL,gsoLS,gsoSS,x,rp,xp=None):
    ...
    rp = xp.asarray(rp)
    nb = x.shape[0]
    fso2e = xp.zeros((3,nb,nb))
    for ic in range(3):
        fso2e[ic] = gsoLL[ic] + gsoLS[ic].dot(x) + x.T.dot(-gsoLS[ic].T) + x.T.dot(gsoSS[ic].dot(x))
        fso2e[ic] = reduce(xp.dot,(rp.T,fso2e[ic],rp))
    return fso2e

def get_fso2e_direct(mol,x,rp,pLL,pLS,pSS,use_1c=False,xp=None):
    '''
    ...
    gsoLL,gsoLS,gsoSS = _accumulate_gso_shell_range(
        mol,pLL,pLS,pSS,0,mol.nbas,use_1c,xp
    )
    return _finalize_fso2e(gsoLL,gsoLS,gsoSS,x,rp,xp)

def _partition_shell_blocks_by_ao(mol,max_block_ao,use_1c=False):
    ao_loc = mol.ao_loc_nr()
    blocks = []
    sh0 = 0
    ao0 = ao_loc[0]
    atom0 = mol.bas_atom(sh0) if use_1c and mol.nbas > 0 else None
    
    if use_1c:
        shells = []
        atoms = []
        for ia in range(mol.natm): # [i,j] i:atoms, j:shells
            tmp_shell = [ish for ish in range(mol.nbas) if mol.bas_atom(ish) == ia]
            tmp_atom = mol.elements[ia]
            if tmp_atom not in atoms:
                atoms.append(mol.elements[ia])
                shells = shells + tmp_shell 
    else:
        shells = list(range(mol.nbas))
    for sh1 in shells:
        if use_1c and sh1 < shells[-1]+1 and mol.bas_atom(sh1) != atom0: # mol.bas_atom(sh1)!=atom0: can't split same atom shell in different blcok
            sh1_id = shells.index(sh1)
            blocks.append((sh0, shells[sh1_id-1]+1)) # next atom, not include sh1
            sh0 = sh1
            ao0 = ao_loc[sh0]
            atom0 = mol.bas_atom(sh0)
            continue
        aocur = ao_loc[sh1] - ao0 # how many AO have store
        if aocur > max_block_ao and sh1 - 1 > sh0 and sh1 < shells[-1]+1: # up to max_block_ao
            blocks.append((sh0, sh1 - 1))
            sh0 = sh1 - 1
            ao0 = ao_loc[sh0]
            atom0 = mol.bas_atom(sh0) if use_1c else None
    blocks.append((sh0, shells[-1] + 1))
    return blocks

def _accumulate_gso_block_range(mol,pLL,pLS,pSS,ib0,ib1,max_block_ao,use_1c=False):
    nb = pLL.shape[0]
    gsoLL = np.zeros((3,nb,nb))
    gsoLS = np.zeros((3,nb,nb))
    gsoSS = np.zeros((3,nb,nb))
    ao_loc = mol.ao_loc_nr()
    blocks = _partition_shell_blocks_by_ao(mol, max_block_ao, use_1c)
    if use_1c:
        block_groups = _block_groups_for_1c(mol, blocks, ib0, ib1)
    else:
        block_groups = [(range(ib0, ib1), range(len(blocks)))]

    for local_blocks, block_ids in block_groups:
        for ib in local_blocks:
            ish0, ish1 = blocks[ib]
            i0, i1 = ao_loc[ish0], ao_loc[ish1]
            di = i1 - i0
            atom_id = mol.bas_atom(ish0) # find atom id base on ish0
            atom_name = mol.elements[atom_id] # atom name, and shell id in atom
            ref_shells = [ish for ish in range(mol.nbas) if mol.bas_atom(ish) == atom_id]
            atom_shell_id = ref_shells.index(ish0)
            if use_1c:
                shifts = []
                for ia in range(mol.natm): # all atoms
                    if mol.elements[ia] != atom_name:
                        continue
                    atom_shells = [ish for ish in range(mol.nbas) if mol.bas_atom(ish) == ia]
                    shift = ao_loc[atom_shells[atom_shell_id]] - i0
                    shifts.append(shift)
            else:
                shifts = [0]
            for jb in block_ids:
                ...
                for kb in block_ids:
                    ...
                    for lb in block_ids:
                        ...
                        for shift in shifts: 
                            for ic in range(3):
                                gsoLL[ic, j0+shift:j1+shift, l0+shift:l1+shift] += -2. * np.einsum(
                                    'lmkn,lk->mn', kblk[ic], pSS[i0+shift:i1+shift,k0+shift:k1+shift]
                                )
                                gsoLS[ic, i0+shift:i1+shift, l0+shift:l1+shift] += -np.einsum(
                                    'mlkn,lk->mn', kblk[ic], pLS[j0+shift:j1+shift,k0+shift:k1+shift]
                                )
                                gsoLS[ic, j0+shift:j1+shift, l0+shift:l1+shift] += -np.einsum(
                                    'lmkn,lk->mn', kblk[ic], pLS[i0+shift:i1+shift,k0+shift:k1+shift]
                                )
                                gsoSS[ic, i0+shift:i1+shift, j0+shift:j1+shift] += -2. * np.einsum(
                                    'mnkl,lk->mn', kblk[ic], pLL[l0+shift:l1+shift,k0+shift:k1+shift]
                                )
                                gsoSS[ic, i0+shift:i1+shift, j0+shift:j1+shift] += -2. * np.einsum(
                                    'mnlk,lk->mn', kblk[ic], pLL[k0+shift:k1+shift,l0+shift:l1+shift]
                                )
                                gsoSS[ic, i0+shift:i1+shift, k0+shift:k1+shift] += 2. * np.einsum(
                                    'mlnk,lk->mn', kblk[ic], pLL[j0+shift:j1+shift,l0+shift:l1+shift]
                                )
    return gsoLL,gsoLS,gsoSS

def _init_gso_direct_par_worker(mol,pLL,pLS,pSS,use_1c=False,num_threads=1):
...
def get_fso2e_direct_par(mol,x,rp,pLL,pLS,pSS,nproc,use_1c=False):
    '''
    ...
    if nproc is None or nproc <= 1 or mol.nbas <= 1:
        return get_fso2e_direct(mol,x,rp,pLL,pLS,pSS,use_1c)
    ...
    gsoLL = np.zeros((3,pLL.shape[0],pLL.shape[1]))
    gsoLS = np.zeros((3,pLL.shape[0],pLL.shape[1]))
    gsoSS = np.zeros((3,pLL.shape[0],pLL.shape[1]))
    logger.info(
        f"direct-par setup: processes={nproc}, tasks={len(tasks)}, "
        f"shells={mol.nbas}, use_1c={use_1c}"
    )
    ...
    with ctx.Pool(
        ...
        initargs=(mol,pLL,pLS,pSS,use_1c,1),
    ) as pool:
    ...
    logger.info("direct-par finalize: assembling fso2e from accumulated Gso blocks")
    return _finalize_fso2e(gsoLL,gsoLS,gsoSS,x,rp)

def get_fso2e_block_par(mol,x,rp,pLL,pLS,pSS,nproc,max_block_ao=_DEFAULT_BLOCK_AO,use_1c=False):
    '''
    ...
    if nproc is None or nproc <= 1:
        gsoLL,gsoLS,gsoSS = _accumulate_gso_block_range(
            mol,pLL,pLS,pSS,0,len(blocks),max_block_ao,use_1c
        )
        return _finalize_fso2e(gsoLL,gsoLS,gsoSS,x,rp)
    ...
    logger.info(
        ...
        f"tasks={len(tasks)}, blocks={len(blocks)}, "
        f"max_block_ao={max_block_ao}, use_1c={use_1c}"
    )
    progress_stride = max(1, len(tasks) // 10)
    done = 0
    par_time0 = time.time()
    ...
    logger.info("block-par finalize: assembling fso2e from accumulated Gso blocks")
    return _finalize_fso2e(gsoLL,gsoLS,gsoSS,x,rp)
...
def get_soDKH1_somf(myhf,
    ...
    here
        ...
        use_1c: one-center approximation for the SOMF 2e term
        backend: 'auto'/'cpu'/'gpu'; GPU uses the serial direct fso2e with
                 CuPy contractions (multiprocessing impls are CPU-only)
        debug: print extra matrix info
    return
        Vso in ao basis
    '''
    logger.info(f"Begin to generate Vso")
    time0 = time.time()
    mode = resolve_backend(backend)
    xp = require_cupy() if mode == "gpu" else np
    #xmol,contr_coeff = myhf.with_x2c.get_xmol(mol) # change basis
    if hasattr(myhf, 'with_x2c'):
        xmol,contr_coeff = myhf.with_x2c.get_xmol(mol) # change basis
    else:
        #myhf = pyscf.scf.sfx2c(myhf)
        #myhf.kernel()
        #xmol,contr_coeff = myhf.with_x2c.get_xmol(mol)
        xmol = mol
        contr_coeff = np.eye(mol.nao_nr())
    ...
    if include_mf2e:
        if mode == "gpu":
            resolved_impl = "direct"
            logger.info(
                "get_soDKH1_somf: GPU backend uses the serial direct fso2e; "
                "multiprocessing impls (direct-par/block-par) are CPU-only"
            )
        else:
    ...
    logger.info(
        ...
        f"use_1c={use_1c}, (nb,nc)={contr_coeff.shape}"
    )
    ...
    if len(dm.shape)==3: 
        ...
        dm = dm/2.
    dm = reduce(np.dot,(contr_coeff,dm,contr_coeff.T))
    ...
    if include_mf2e:
        ...
        try:
            ...
            if resolved_impl == 'full':
                ...
                if debug:
                    for ic in range(3):
                        logger.info(f"ic={ic, np.linalg.norm(kint[ic]+kint[ic].transpose(2,3,0,1))}")
                fso2e_time0 = time.time()
                if use_1c:
                    fso2e = get_fso2e_1c(kint,xmol,x,rp,pLL,pLS,pSS)
                else:
                ...
                fso2e = get_fso2e_direct(xmol,x,rp,pLL,pLS,pSS,use_1c,xp)
                mf2e_time1 = time.time()
                logger.info(f"mf2e timing ({resolved_impl}): total={mf2e_time1-mf2e_time0:.2f}s")
            elif resolved_impl == 'direct-par':
                fso2e = get_fso2e_direct_par(xmol,x,rp,pLL,pLS,pSS,nproc,use_1c)
                mf2e_time1 = time.time()
                logger.info(f"mf2e timing ({resolved_impl}, nproc={nproc}): total={mf2e_time1-mf2e_time0:.2f}s")
            elif resolved_impl == 'block-par':
                fso2e = get_fso2e_block_par(xmol,x,rp,pLL,pLS,pSS,nproc,use_1c=use_1c)
                mf2e_time1 = time.time()
                logger.info(f"mf2e timing ({resolved_impl}, nproc={nproc}): total={mf2e_time1-mf2e_time0:.2f}s")
            else:
    ...
    if debug:
        for ic in range(3):
            tmp = hso1e[ic]
            logger.info(f"{ic} hso1e  norm={np.linalg.norm(asnumpy(tmp)):.12e}, "
                f"Norm(tmp+tmp.T)={np.linalg.norm(asnumpy(tmp + tmp.T)):.12e}")
            if include_mf2e:
                tmp = fso2e[ic]
                logger.info(f"{ic} fso2e  norm={np.linalg.norm(asnumpy(tmp)):.12e}, "
                    f"Norm(tmp+tmp.T)={np.linalg.norm(asnumpy(tmp + tmp.T)):.12e}")
                tmp = xp.asarray(hso1e[ic]) + fso2e[ic]
                logger.info(f"{ic} vso2e  norm={np.linalg.norm(asnumpy(tmp)):.12e}, "
                    f"Norm(tmp+tmp.T)={np.linalg.norm(asnumpy(tmp + tmp.T)):.12e}")

    cc = xp.asarray(contr_coeff)
    VsoDKH1contr = xp.zeros((3,nc,nc))
    for ic in range(3):
        VsoDKH1contr[ic] = reduce(xp.dot,(cc.T,VsoDKH1[ic],cc))
    if debug:
        for ic in range(3):
            tmp = VsoDKH1contr[ic]
            logger.info(f"{ic} VsoDKH1contr  norm={np.linalg.norm(asnumpy(tmp)):.12e}, "
                f"Norm(tmp+tmp.T)={np.linalg.norm(asnumpy(tmp + tmp.T)):.12e}")
    time1 = time.time()
    logger.info(f"End of generating Vso, cost time={time1-time0:.2f}s")
    return VsoDKH1contr
