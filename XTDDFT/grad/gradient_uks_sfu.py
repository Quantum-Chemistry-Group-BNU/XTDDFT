...
from ...utils.hxc_part import mcfun_eval_xc_adapter_sf
...
def grad_elec(td, atmlst=None, max_memory=2000, verbose=logger.INFO):
    ...
    time1 = log.timer('Z-vector using UCPHF solver', *time1)
    ...
    dmz1dvva = (dmz1dvva + dmz1dvva.T) * 0.5
    dmz1doob = (dmz1doob + dmz1doob.T) * 0.5
    oo0a = orbo_a @ orbo_a.T
    oo0b = orbo_b @ orbo_b.T
    as_dm1 = oo0a + oo0b + (dmz1dvva + dmz1doob) * 0.5  # follow CPU version naming convention
    ...
    if gpu:
        ...
        dveff1_0 = gpu_rhf_grad.contract_h1e_dm(mol, veff1_0_a, oo0a + dmz1dvva * 0.5, hermi=0)
        dveff1_0 += gpu_rhf_grad.contract_h1e_dm(mol, veff1_0_b, oo0b + dmz1doob * 0.5, hermi=0)
