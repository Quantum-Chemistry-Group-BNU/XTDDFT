...
def grad_elec(td, atmlst=None, max_memory=2000, verbose=logger.INFO):
    ...
    if with_k:
        ...
        wvoa -= lib.einsum('jk,jc->ck', veff0mo[:nc, :(nc+no)], v)  # 1/2 Q_{ai}^{\alpha}
    ...
    if with_k:
        dm = (oo0a, dmz1dvva + dmz1dvva.T, oo0b, dmz1doob + dmz1doob.T)
        # (4,3,nao,nao), 2nd index is derivate respect to coordinate
    ...
    for k, ia in enumerate(atmlst):
        ...
        de[k] += lib.einsum('xpq,pq->x', veff1a[0, :, p0:p1], dmz1dvva[p0:p1]) * 0.5
        de[k] += lib.einsum('xpq,pq->x', veff1b[0, :, p0:p1], dmz1doob[p0:p1]) * 0.5
        de[k] += lib.einsum('xpq,qp->x', veff1a[0, :, p0:p1], dmz1dvva[:, p0:p1]) * 0.5
