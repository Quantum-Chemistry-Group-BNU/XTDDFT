...
class XsTDA:
    ...
    def my_stda(self):
        ...
        if self.cas:
            ...
            vthr = (deps * 2 + somo_low)
            othr = (somo_high - deps * 2)
            assert vthr > othr
        ...
        for (shl0,shl1,p0,p1) in ao_slices:
            qAk_aa.append(np.einsum('mi, ma->ia', C_prime[p0:p1, :nocc_a], C_prime[p0:p1, nocc_a:]))  # exchange
            qAj_aa.append(np.einsum('mi, mj->ij', C_prime[p0:p1, :nocc_a], C_prime[p0:p1, :nocc_a]))  # coulomb
            qAk_bb.append(np.einsum('mi, ma->ia', C_prime[p0:p1, :nocc_b], C_prime[p0:p1, nocc_b:]))  # exchange
            qAj_bb.append(np.einsum('mi, mj->ij', C_prime[p0:p1, :nocc_b], C_prime[p0:p1, :nocc_b]))  # coulomb
            qBj_aa.append(np.einsum('ma, mb->ab', C_prime[p0:p1, nocc_a:], C_prime[p0:p1, nocc_a:]))  # coulomb
            qBj_bb.append(np.einsum('ma, mb->ab', C_prime[p0:p1, nocc_b:], C_prime[p0:p1, nocc_b:]))  # coulomb
        ...
        if self.truncate:
            ...
            for i, j in zip(pcsfcv_a[0], pcsfcv_a[1]):
                ...
                iajbcv_a += self._cvaacvaa_ndc(i, j, ncsfcv_a, nc, no, nv, si, fij_a2, fij_b2, fab_a2, fab_b2)
                ...
                iajbkcv_b += self._cvaacvbb_ndc(i, j, ncsfcv_b, nc, no, nv, si, fij_a2, fij_b2, fab_a2, fab_b2)
            ...
            for i, j in zip(pcsfcv_b[0], pcsfcv_b[1]):
                ...
                iajbkcv_a += self._cvaacvbb_ndc(i, j, ncsfcv_a, nc, no, nv, si, fij_a2, fij_b2, fab_a2, fab_b2)  # _cvaacvbb = _cvbbcvaa
                ...
                iajbcv_b += self._cvbbcvbb_ndc(i, j, ncsfcv_b, nc, no, nv, si, fij_a2, fij_b2, fab_a2, fab_b2)
                iajbline = np.concatenate((iajbkcv_a, iajbkov_a, iajbco_b, iajbcv_b), axis=0)
        ...
        for i, a, p in zip(pscsfcv_a_i, pscsfcv_a_a, range(lcva)):
            A[p, :lcva] -= np.einsum('nmp, nmp, nm->p', qAj_aa[:, None, i, pscsfcv_a_i], qBj_aa[None, :, a, pscsfcv_a_a], gamma_j)
        ...
        for i, a, p in zip(pscsfcv_b_i, pscsfcv_b_a, range(lcvb)):
            ...
            A[lcva+lova+lcob+p, lcva+lova+lcob:] -= np.einsum('nmp, nmp, nm->p', qAj_bb[:, None, i, pscsfcv_b_i], qBj_bb[None, :, no+a, no+pscsfcv_b_a], gamma_j)
...
if __name__ == "__main__":
    mol = gto.M(
        ...
        # atom=atom.ch2o_p_Xylene,
        # atom=atom.ch2o_Toluene,
        # atom=atom.ch2o_DiIsoPropylEther,
        # atom=atom.ch2o_DiethylEther,
        # atom=atom.ch2o_Chloroform,
    ...
    # mf.with_solvent.eps = 2.2705  # for p-Xylene 对二甲苯
    # mf.with_solvent.eps = 2.3741  # for Toluene 甲苯
    # mf.with_solvent.eps = 3.38  # for DiIsoPropylEther 二异丙醚
    # mf.with_solvent.eps = 4.2400  # for DiethylEther 乙醚
    # mf.with_solvent.eps = 4.7113  # for Chloroform 氯仿
    ...
    xstda.truncate = 20
    xstda.correct = False
    e_eV, os, rs, v, pscsf = xstda.my_stda()
