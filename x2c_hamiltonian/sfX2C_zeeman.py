...
def get_Tsfsd(mol):
    ...
    if debug:
        ...
        print('debug tsf,tsd')
        for ic in range(3):
            print(f'ic={ic}', numpy.linalg.norm(tsf[ic]), numpy.sum(tsf[ic]))
            print(tsf[ic][:,0])

        for ic in range(3):
            for jc in range(3):
                print(f'ic={ic}, jc={jc}', numpy.linalg.norm(tsd[ic,jc]), numpy.sum(tsd[ic,jc]))
                print(tsd[ic,jc][:3,0])
                print(tsd[ic,jc][0,:3])
    return tsf,tsd
...
def get_mag(a4,sinv,x,rp,h1e,tsf,wsf,sgn):
    tmp1 = reduce(numpy.dot,(rp.T,tsf.dot(x)+x.T.dot(tsf)+(x.T.dot(a4*wsf-tsf)).dot(x),rp))
...
def get_zeeman(myhf,mol,c,org):
    ...
    print(f"get_zeeman (np,nc)={contr_coeff.shape}")
