...
def get_fso2e_1c(kint,mol,x,rp,pLL,pLS,pSS):
    ...
    for a0, a1 in _atom_ao_slices(mol):
        for ic in range(3):
            ...
            gsoLL[ic, a0:a1, a0:a1] += -2. * numpy.einsum(
                'lmkn,lk->mn', ksub, pSS[a0:a1,a0:a1]
            )
            gsoLS[ic, a0:a1, a0:a1] += -numpy.einsum(
                'mlkn,lk->mn', ksub, pLS[a0:a1,a0:a1]
            )
            gsoLS[ic, a0:a1, a0:a1] += -numpy.einsum(
                'lmkn,lk->mn', ksub, pLS[a0:a1,a0:a1]
            )
            gsoSS[ic, a0:a1, a0:a1] += -2. * numpy.einsum(
...
def _accumulate_gso_shell_range(mol,pLL,pLS,pSS,ish0,ish1,use_1c=False):
    nb = pLL.shape[0]
    gsoLL = numpy.zeros((3,nb,nb))
    gsoLS = numpy.zeros((3,nb,nb))
    gsoSS = numpy.zeros((3,nb,nb))
...
def get_fso2e_direct(mol,x,rp,pLL,pLS,pSS,use_1c=False):
    '''
    ...
    gsoLL,gsoLS,gsoSS = _accumulate_gso_shell_range(
        mol,pLL,pLS,pSS,0,mol.nbas,use_1c
    )
    return _finalize_fso2e(gsoLL,gsoLS,gsoSS,x,rp)
            ...
            for jb in block_ids:
                ...
                for kb in block_ids:
                    ...
                    for lb in block_ids:
                        ...
                        for shift in shifts: 
                            for ic in range(3):
                                gsoLL[ic, j0+shift:j1+shift, l0+shift:l1+shift] += -2. * numpy.einsum(
                                    'lmkn,lk->mn', kblk[ic], pSS[i0+shift:i1+shift,k0+shift:k1+shift]
                                )
                                gsoLS[ic, i0+shift:i1+shift, l0+shift:l1+shift] += -numpy.einsum(
                                    'mlkn,lk->mn', kblk[ic], pLS[j0+shift:j1+shift,k0+shift:k1+shift]
                                )
                                gsoLS[ic, j0+shift:j1+shift, l0+shift:l1+shift] += -numpy.einsum(
                                    'lmkn,lk->mn', kblk[ic], pLS[i0+shift:i1+shift,k0+shift:k1+shift]
                                )
                                gsoSS[ic, i0+shift:i1+shift, j0+shift:j1+shift] += -2. * numpy.einsum(
                                    'mnkl,lk->mn', kblk[ic], pLL[l0+shift:l1+shift,k0+shift:k1+shift]
                                )
                                gsoSS[ic, i0+shift:i1+shift, j0+shift:j1+shift] += -2. * numpy.einsum(
                                    'mnlk,lk->mn', kblk[ic], pLL[k0+shift:k1+shift,l0+shift:l1+shift]
                                )
                                gsoSS[ic, i0+shift:i1+shift, k0+shift:k1+shift] += 2. * numpy.einsum(
...
def get_soDKH1_somf(myhf,
    ...
    if hasattr(myhf, 'with_x2c'):
        ...
        xmol = mol
        contr_coeff = numpy.eye(mol.nao_nr())
    ...
    if include_mf2e:
        ...
        try:
            ...
            if resolved_impl == 'full':
                ...
                fso2e = get_fso2e_direct(xmol,x,rp,pLL,pLS,pSS,use_1c)
