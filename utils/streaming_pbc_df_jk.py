...
XTDDFT.utils.df_cderi_cache.  The J/K contraction reads auxiliary-function
...
STREAMING_PBC_DF_JK_VERSION = "2026-06-11-gpu-overlap-ewald"
...
def _read_cache_omega(cache_path, *, group="pbc_gamma"):
    with h5py.File(Path(cache_path), "r") as handle:
        return _omega_key(handle[group].attrs.get("omega", 0.0))


def _assert_cache_omega(cache_path, omega, *, group="pbc_gamma"):
    expected = _omega_key(omega)
    actual = _read_cache_omega(cache_path, group=group)
    if abs(actual - expected) > 1e-12:
        raise RuntimeError(
            "streaming DF CDERI cache omega mismatch: "
            f"{cache_path} has omega={actual}, requested omega={expected}. "
            "Remove the stale cache file or rebuild it with the current helper."
        )


def _remove_stale_cache_for_omega(cache_path, omega, *, group="pbc_gamma"):
    ...
    if abs(actual - expected) <= 1e-12:
        return
    print(
        "Removing stale streaming DF CDERI cache "
        f"{cache_path}: cached omega={actual}, requested omega={expected}"
    )
    cache_path.unlink()
...
def _apply_ewald_exxdiv_for_gamma(vk, dm, overlap, madelung, xp):
    """Apply PySCF's gamma-point Ewald exchange-divergence correction."""

    if vk is None:
        return None
    dm_work, single_dm = _normalize_dm(dm, xp)
    vk_work, vk_single = _normalize_dm(vk, xp)
    if single_dm != vk_single:
        raise ValueError("vk and dm stack dimensions are inconsistent")

    s = xp.asarray(overlap)
    correction = xp.asarray(madelung) * xp.stack([
        reduce(xp.dot, (s, dm_i, s)) for dm_i in dm_work
    ])
    vk_work = vk_work + correction
    return _restore_shape(vk_work, single_dm)


def _should_apply_ewald_exxdiv(*, with_k, exxdiv) -> bool:
    return bool(with_k and isinstance(exxdiv, str) and exxdiv.lower() == "ewald")


def _gamma_madelung(cell, *, tools_module=None):
    if tools_module is None:
        from pyscf.pbc import tools as tools_module
...
def _gamma_overlap(cell, *, kpts=None, use_gpu=False, int1e_module=None):
    ...
    if int1e_module is None:
        overlap = cell.pbc_intor("int1e_ovlp", hermi=1, kpts=kpts)
    else:
        overlap = int1e_module.int1e_ovlp(cell, kpts=kpts)

    if getattr(overlap, "ndim", 0) == 3 and overlap.shape[0] == 1:
        return overlap[0]
    return overlap
    ...
    *,
    ...
    with h5py.File(Path(cache_path), "r") as handle:
        ...
        for p0 in range(0, naux, aux_block_size):
            ...
            vj_block, vk_block = streaming_jk_from_cderi_block(
                cderi, dm_work, with_j=with_j, with_k=with_k, xp=xp
            )
            if vj_block is not None:
                vj += vj_block
            if vk_block is not None:
                vk += vk_block
...
def streaming_jk_from_cderi_block(cderi, dm_work, *, with_j=True, with_k=True, xp=np):
    """Contract one expanded CDERI block against stacked density matrices."""
    ...
    if with_k:
        tmp = xp.einsum("Lpq,xqr->Lxpr", cderi, dm_work, optimize=True)
        vk = xp.einsum("Lxpr,Lsr->xps", tmp, cderi, optimize=True)
    return vj, vk


def _load_xtddft_cache_helpers():
    from XTDDFT.utils.df_cderi_cache import (
        DFCderiCacheConfig,
...
def install_streaming_df_jk(
    ...
    *,
    ...
    for omega in omegas:
        ...
        if build_cache:
            _remove_stale_cache_for_omega(cache_path, omega)
        ...
        _assert_cache_omega(cache_path, omega)
    ...
    def streaming_get_jk(
        ...
        if not _is_gamma_kpts(kpts) or not _is_gamma_kpts(kpts_band):
