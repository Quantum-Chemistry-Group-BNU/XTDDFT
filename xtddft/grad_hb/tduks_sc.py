...
def excited_energy(atom, spec, state, xc='b3lyp'):
    ...
    td = UTDA(mol, mf, nstates=max(state, 1)+2)
    td.kernel()
...
def grad_elec(td, atmlst=None, max_memory=2000, verbose=logger.INFO):
    ...
    for k, ia in enumerate(atmlst):
        ...
        de[k] += np.einsum('xpq,pq->x', veff1a[0,:,p0:p1], dmz1dooa[p0:p1])
        de[k] += np.einsum('xpq,pq->x', veff1b[0,:,p0:p1], dmz1doob[p0:p1])
        de[k] += np.einsum('xpq,qp->x', veff1a[0,:,p0:p1], dmz1dooa[:,p0:p1])
        de[k] += np.einsum('xpq,qp->x', veff1b[0,:,p0:p1], dmz1doob[:,p0:p1])

        de[k] -= np.einsum('xpq,pq->x', s1[:,p0:p1], im0[p0:p1])
        de[k] -= np.einsum('xqp,pq->x', s1[:,p0:p1], im0[:,p0:p1])

        de[k] += np.einsum('xij,ij->x', veff1a[1,:,p0:p1], oo0a[p0:p1])
        de[k] += np.einsum('xij,ij->x', veff1b[1,:,p0:p1], oo0b[p0:p1])
        de[k] += np.einsum('xij,ij->x', veff1a[2,:,p0:p1], dmxa[p0:p1,:])*2
        de[k] += np.einsum('xij,ij->x', veff1b[2,:,p0:p1], dmxb[p0:p1,:])*2
        de[k] += np.einsum('xji,ij->x', veff1a[3,:,p0:p1], dmxa[:,p0:p1])*2
        de[k] += np.einsum('xji,ij->x', veff1b[3,:,p0:p1], dmxb[:,p0:p1])*2
...
if __name__ == "__main__":
    ...
    td = UTDA(mol, mf, nstates=3)
    td.kernel()

    sfu_uks_td = SC_gradient(td)
    sfu_uks_td.kernel()
