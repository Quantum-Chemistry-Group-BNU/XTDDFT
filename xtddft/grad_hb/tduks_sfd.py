...
def excited_energy(atom, spec, state, xc='b3lyp', method=1, cs=50):
    ...
    td = XSF_TDA(mf, method=method, davidson=True)
...
class SFD_gradient(uhf_grad.Gradients):
    ...
    def grad_elec(self, atmlst=None):
        if self.v is None:
            ...
            self.base.kernel(states=max(self.state, 1)+2)
...
if __name__ == "__main__":
    ...
    td = XSF_TDA(mf, method=1, davidson=True)
    td.collinear_samples = 50
    td.kernel(nstates=max(1, 1) + 2)

    sfu_uks_td = SFD_gradient(td, method=1)
    sfu_uks_td.kernel()
