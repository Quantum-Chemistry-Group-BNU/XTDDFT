...
def excited_energy(atom, spec, state, xc='b3lyp', method=1, cs=50):
    ...
    td = XSF_TDA(mf, method=method, davidson=True, SA=3)
...
def grad_elec(td, fglobal=None, fit=True,d_lda=0.3, atmlst=None, max_memory=2000, verbose=logger.INFO):
    ...
    if re:
        v_oo = lib.einsum('xy,y->x', sftda.vects, v[nc*na+no*nv:])
    else:
        v_oo = v[nc*na+no*nv:]
    ...
    c1 = 1 / si
    c2 = 2 / (2 * si - 1)
    c3 = np.sqrt((2 * si + 1)/(2 * si)) - 1
    c4 = np.sqrt((2 * si + 1)/(2 * si - 1)) - 1
    c5 = np.sqrt((2 * si)/(2 * si - 1)) - 1
    c6 = 1 / np.sqrt(2 * si * (2 * si - 1))
    dmcc = lib.einsum('aj,ai->ij', v[no:, :nc], v[no:, :nc]) * c1
    dmcc += lib.einsum('tj,ti->ij', v[:no, :nc], v[:no, :nc]) * c2
    dmvv = lib.einsum('bi,ai->ab', v[no:, :nc], v[no:, :nc]) * c1
    dmvv += lib.einsum('bt,at->ab', v[no:, nc:ni], v[no:, nc:ni]) * c2
    # dmvc = v[no:, :nc] * np.trace(v[:no, nc:ni]) * c1 * (c4 + 1)  # always zero
    dmoca = -lib.einsum('au,ai->ui', v[no:, nc:ni], v[no:, :nc]) * c3
    dmoca -= lib.einsum('tu,ti->ui', v[:no, nc:ni], v[:no, :nc]) * c5
    dmocb = v[:no, :nc] * np.trace(v[:no, nc:ni]) * c6
    dmvoa = -v[no:, nc:ni] * np.trace(v[:no, nc:ni]) * c6
    dmvob = lib.einsum('ui,ai->au', v[:no, :nc], v[no:, :nc]) * c3
    dmvob += lib.einsum('ut,at->au', v[:no, nc:ni], v[no:, nc:ni]) * c5
    scorrao = orbo_a[:, nc:ni] @ (np.eye(no)/2) @ orbo_a[:, nc:ni].T  # s correct ao

    dmS = np.zeros((nao, nao))  # contract with F^{S}
    dmHFa = np.zeros_like(dmS)  # contract with F^{HF,\alpha}
    dmHFb = np.zeros_like(dmS)  # contract with F^{HF,\beta}
    dmS[:nc, :nc] = dmcc
    dmS[ni:, ni:] = dmvv
    ...
    if with_k:
        vj, vk = mf.get_jk(mol, (dmzooa, dmzvvb), hermi=1)  # g_{\mu\nu}^{\sigma}[T]
        vk = vk * hyb
        ...
        wvoa += lib.einsum('ba,bi->ai', veff0mo[nc:, ni:], v)  # part of 1/2 Q_{ia}^{\alpha}
        wvoa += lib.einsum('ij,aj->ai', dooa, fockamo[ni:, :ni])  # part of 1/2 Q_{ia}^{\alpha}
        wvob -= lib.einsum('ij,aj->ai', veff0mo[:nc, :ni], v)  # 1/2 Q_{ai}^{\beta}
        wvob -= lib.einsum('ab,ib->ai', dvvb, fockbmo[:nc, nc:])  # 1/2 Q_{ai}^{\beta}
    else:
        vj = mf.get_j(mol, (dmzooa, dmzvvb), hermi=1)  # (2, nao, nao)
    ...
    wcca += lib.einsum('ik,jk->ij', dooa, fockamo[:ni, :ni])
    wcca += lib.einsum('aj,ai->ij', veff0mo[nc:, :ni], v)
    wvvb = lib.einsum('ac,bc->ab', dvvb, fockbmo[nc:, nc:])
    wvvb += lib.einsum('bi,ai->ab', veff0mo[nc:, :ni], v)
    ...
    dq[ni:, :] += lib.einsum('ip,ai->ap', x_cv_i, v[no:, :nc])
    dq[:nc, :] += lib.einsum('ap,ai->ip', x_cv_a, v[no:, :nc])
    dq[:nc, :] += lib.einsum('tp,ti->ip', x_co_t, v[:no, :nc])
    dq[nc:ni, :] += lib.einsum('ip,ti->tp', x_co_i, v[:no, :nc])
    dq[nc:ni, :] += lib.einsum('ap,at->tp', x_ov_v, v[no:, nc:ni])
    dq[ni:, :] += lib.einsum('tp,at->ap', x_ov_t, v[no:, nc:ni])
    dq[nc:ni, :] += lib.einsum('vp,vt->tp', x_oo_t, v[:no, nc:ni])
    dq[nc:ni, :] += lib.einsum('tp,vt->vp', x_oo_u, v[:no, nc:ni])
    ...
    # # Z, instead of 1/2 Z
    z = lib.solve(matvec, w, tol=1e-12, max_cycle=td.cphf_max_cycle,
    ...
    im0a[:ni, ni:] += lib.einsum('ba,bi->ia', veff0mo[nc:, ni:], v)
    ...
    im0b[nc:, :nc] += lib.einsum('ab,ib->ai', dvvb, fockbmo[:nc, nc:])  # ROKS particle term
    im0b[nc:, :nc] += lib.einsum('ij,aj->ai', veff0mo[:nc, :ni], v)
    im0 = im0a + im0b
...
if __name__ == "__main__":
    ...
    td = XSF_TDA(mf, method=1, davidson=True, SA=3)
    td.collinear_samples = 50
    td.kernel(nstates=max(1, 1) + 2)

    sfu_roks_td = SFD_gradient(td, method=1)
    sfu_roks_td.kernel()

    # g_fd = fd_gradient(ch2o, 1, xc=xc, method=1)
    # print('finite-diff:\n', g_fd)
