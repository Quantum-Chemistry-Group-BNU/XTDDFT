...
class UCIS:
    ...
    def kernel(self):
        ...
        for i in range(self.nstates):
            x_a = self.v[:, i][:self.nc*self.nv + self.no*self.nv].reshape(-1)  # alpha振幅
            x_b = self.v[:, i][self.nc*self.nv + self.no*self.nv:].reshape(-1)   # beta振幅
            self.xy.append( (x_a, x_b) )
